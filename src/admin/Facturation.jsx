...
export default function Facturation() {
  ...
  async function save() {
    ...
    if (editInv) {
      await upsert('invoices', { ...editInv, ...record }, 'invoices')
    } else {
      await insert('invoices', { ...record, id: `inv_${Date.now()}` }, 'invoices')
    }
    setAddOpen(false)
  }
  ...
  async function markRetard(inv) {
    await upsert('invoices', { ...inv, statut: 'en_retard' }, 'invoices')
  }
    ...
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          ...
          {INV_CATS.map(cat => (
            <button key={cat.id} style={{ ...st.btn(filterCat === cat.id ? 'primary' : 'ghost', 'sm'), background: filterCat === cat.id ? cat.color : 'transparent', color: filterCat === cat.id ? '#fff' : C.sub }}
                  ...
                  return (
                    <div key={inv.id} style={{ ...st.card(14), marginBottom: 8 }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 6 }}>
                        <div style={{ flex: 1 }}>
                          <div style={{ fontWeight: 700, fontSize: 13, marginBottom: 3 }}>{inv.libelle}</div>
                          <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap', alignItems: 'center' }}>
                            ...
                            {inv.beneficiaire && <span style={st.sub}>{inv.beneficiaire}</span>}
                          </div>
                        </div>
                      ...
                      {inv.statut === 'payé' && inv.date_paiement && (
                        <div style={{ fontSize: 11, color: C.green, marginBottom: 6 }}>✓ Payé le {new Date(inv.date_paiement).toLocaleDateString('fr-FR')}</div>
                      )}
                      <div style={{ display: 'flex', gap: 5 }}>
                        {inv.statut !== 'payé' && <button style={st.btn('green', 'xs')} onClick={() => markPaid(inv)}>✓ Payé</button>}
                        {inv.statut === 'en_cours' && <button style={st.btn('danger', 'xs')} onClick={() => markRetard(inv)}>⚠ Retard</button>}
        ...
        <Modal title={editInv ? 'Modifier la facture' : '+ Nouvelle facture'} onClose={() => setAddOpen(false)} width={500}>
          <div style={st.g2}>
            <div style={{ gridColumn: '1/-1' }}>
              <Field label="Libellé"><input style={st.input} value={form.libelle} onChange={e => f('libelle', e.target.value)} placeholder="Ex : Salaire VA Samy"/></Field>
            </div>
